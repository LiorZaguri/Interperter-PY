...
* **Tuple Operations**:
    ...
    Tests various tuple operations such as appending, removing, and checking contents.
...
*   **Addition `ADD(n, m)`**: Adds two numbers, concatenates two strings, or merges two lists or sets.
*   **Subtraction `SUB(n, m)`**: Subtracts two numbers (n - m)
*   **Multiplication `MUL(n, m)`**: Multiplies two numbers (n * m)
*   **Division `DIVIDE(n, m)`**: Returns the division of n by m (n / m)
*   **Floor Division `FLOOR_DIVIDE(n, m)`**: Returns the floor division of n by m (n // m)
*   **Power `POWER(n, m)`**: Returns n raised to the power of m (n^m)
*   **Square `SQUARE(n)`**: Returns the square of a number. (n^2)
*   **Factorial `FACTORIAL(n)`**: Returns the factorial of a number. (n!)
*   **Absolute Value `ABS(n)`**: Returns the absolute value of a number. (|n|)
*   **Modulo `MOD(n, m)`**: Returns the remainder of the division of n by m. (n % m)
*   **Square Root `SQRT(n)`**: Returns the square root of a number. (√n)

#### Logical Operations

The interpreter provides basic logical operations:

*   **Equal `EQUAL(e1, e2)`**: Checks if two elements are equal. The elements can be numbers, strings, lists, or sets. For lists and sets, the order of elements is considered.
*   **Not Equal `NOT_EQUAL(e1, e2)`**: Checks if two elements are not equal.
*   **Greater Than `GREATER(e1, e2)`**: Checks if one element is greater than another. (e1 > e2)
*   **Greater Than or Equal `GREATER_EQUAL(e1, e2)`**: Checks if one element is greater than or equal to another.
*   **Smaller Than `SMALLER(e1, e2)`**: Checks if one element is smaller than another.
*   **Smaller Than or Equal `SMALLER_EQUAL(e1, e2)`**: Checks if one element is smaller than or equal to another.
*   **Logical OR `OR(bool1, bool2)`**: Performs a logical OR operation on two boolean values.
*   **Logical AND `AND(bool1, bool2)`**: Performs a logical AND operation on two boolean values.
*   **Logical NOT `NOT(bool)`**: Performs a logical NOT operation on a boolean value.

#### String Operations

The interpreter supports a variety of string manipulations:

*   **Concatenation `CONCAT(string1, string2)`**: Concatenates two strings.
*   **Splitting `SPLIT(string, delimiter)`**: Splits a string into a list of substrings based on a delimiter.
*   **Replacing `REPLACE(string, old, new)`**: Replaces all occurrences of a substring in a string with another substring.
*   **Reversing `REVERSE(string)`**: Reverses a string.
*   **Uppercase Check `ISUPPER(string)`**: Checks if all characters in a string are uppercase.
*   **Lowercase Check `ISLOWER(string)`**: Checks if all characters in a string are lowercase.

#### List Operations

The interpreter allows various operations on lists:

*   **Appending `APPEND(list, element)`**: Adds an element to the end of a list.
*   **Removing `REMOVE(list, element)`**: Removes an element from a list.
*   **Length `LENGTH(list)`**: Returns the length of a list.
*   **Contains `CONTAINS(list, element)`**: Checks if a list contains a specific element.
*   **Index `INDEX(list, element)`**: Returns the index of an element in a list.
*   **Get Element `GET(list, index)`**: Returns the element at a specific index in a list.
